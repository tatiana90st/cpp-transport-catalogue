...
void Process(TransportCatalogue& cat, std::istream& input, std::ostream& out) {
	...
	for (int i = 0; i < q_amount; ++i) {
		...
		case query_sort::QueryType::BUS:
			detail::PrintBusRequestResult(q.text, cat, out);
			out << std::flush;
			break;
		case query_sort::QueryType::STOP:
			detail::PrintStopRequestResult(q.text, cat, out);
			out << std::flush;
			break;
